...
- 点击文本直接粘贴；
- 长按文本可选择复制；复制或粘贴后自动回到中文主键盘，拆词页面也遵循此行为；
- 编辑或删除条目；
- 使用图钉固定地址、邮箱等常用内容；
...
- 复制或粘贴后明确切换回中文主键盘布局，不再停留在剪贴板页或保留数字、符号页
- 移除会阻止自动返回的旧剪贴板开关，行为与个人版输入流程保持一致

### 1.1.14

- 复制或粘贴后自动回到中文主键盘，覆盖普通条目与拆词页面
- 普通文本条目的长按菜单新增“复制”入口
- 拆词页“发送”按钮改为“粘贴”，准确描述插入文本的行为
- 离开拆词页面时取消后台加载，避免完成后继续更新已关闭的页面

### 1.1.13

- 数字键盘左侧连续符号轨道的四个首屏符号严格等高，每项占轨道高度的四分之一
- 新输入会话会立即清除旧候选、局部邮箱建议和滚动位置，避免跨应用残留
- 修复邮箱补全期间退格可能重复删除文本，以及输入结束后长按退格任务未及时停止的问题
- 候选栏宽度会随窗口和横竖屏尺寸重新计算，保持填充模式布局稳定
- 剪贴板搜索遵循“全部、固定、本地、媒体、远程”当前分类
- 输入统计在编辑会话结束和服务退出时主动落盘，降低异常终止时丢失最近统计的概率
- 释放未使用的预热键盘视图，避免主题切换或服务退出后保留监听器
- 修正非 ASCII 字面字符传入输入引擎时的 Unicode 到 KeySym 映射

### 1.1.12

- 数字键盘左侧符号区改为统一底色的连续滚动轨道，不再呈现为多个独立按键
- 轨道首屏在三行数字键的高度内显示四个符号，并以淡分割线提示列表结构
- 保留惯性纵向滚动，可继续查看并输入 `( ) . @ %`
