...
```text
org.fcitx5.slei.android.fx-1.0.1-arm64-release.apk
```
...
`arm64-v8a` 的 `versionCode` 计算方式是 `baseVersionCode * 10 + 2`；`1.0.1` 使用 `1012`。

### 1.0.1 发行说明

- 修复从旧版本升级后，中文键盘 `V` 上滑虽输入 `_`、键帽却仍显示横线的问题；迁移只修改默认布局中的这一项，不覆盖其他自定义。
- 剪贴板条目新增始终可见的图钉入口，点击即可固定/取消固定；固定项优先显示且不会被数量或过期清理删除，适合保存地址等常用文本。
- 增加精确、离线的常见误读词映射：输入 `mo ban`（小鹤双拼对应键码也支持）可找到“模板”。使用词组级别纠音，不会把所有 `mo/mu` 候选混在一起。
- 候选横栏由整表刷新改为局部更新，减少连续输入时的布局与对象抖动。
- 符号和 Emoji 项的显示转换改为分类切换时预计算，滚动期间不再重复做相同转换；保留真实连续滚动和任意位置停止。
- 剪贴板图片缩略图缓存从“固定条目数”改为 8 MiB 字节预算，并在系统低内存时主动释放；字体缓存也响应内存压力。
- 可重建的临时文件优先写入 Android 应用专属外部缓存，按 7 天和 64 MiB 预算自动清理；布局、字体、用户词库继续使用应用专属外部文件目录。
- 用户设置、输入历史和剪贴板数据库仍保存在内部应用数据中，避免外部存储暂时不可用或被清理时丢失关键数据。
